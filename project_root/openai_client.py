...
class OpenAIClient:
    ...
    def generate_user_reply(
        ...
        *,
        ...
        """Generate a short reply to a user message."""
        # Persona is presentation-only and must not affect decision logic.
        context_block = "\n".join(
            f"- {text}" for text in context_messages if text.strip()
        )
        prompt = (
            "Ты участник живого Telegram-чата. "
            "Ответь кратко и по-человечески (1–2 предложения). "
            "Не пиши как эксперт, не как статья, не как бот.\n\n"

            "Требования:\n"
            "- Пиши так, будто ты реально читаешь чат и реагируешь на конкретное сообщение.\n"
            "- Можно согласиться, усомниться или уточнить.\n"
            "- Допустим лёгкий спор или эмоциональная реакция.\n"
            "- Не использовать фразы типа: 'Это может', 'Это может привести', "
            "'Это может повлиять'.\n"
            "- Не использовать абстрактные формулировки вроде "
            "'общественное восприятие', 'социальный эффект'.\n"
            "- Без морализаторства и без канцелярита.\n"
            "- Без ссылок, без хэштегов, без призывов.\n\n"
            "- Не все ответы должны быть серьёзными.\n"
            "- Иногда допустим короткий комментарий без развернутого объяснения.\n"
