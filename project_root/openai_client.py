...
# Вариативность формулировок вопроса ведущего (админ к новости): не только «Как думаете», «Что вы об этом думаете?»
ADMIN_QUESTION_PHRASING_POOL = [
...
class OpenAIClient:
    ...
    def generate_discussion_messages(
        ...
        *,
        ...
        # Случайная выборка формулировок вопроса ведущего — чтобы админ не повторял одни и те же фразы
        question_phrasing_sample = random.sample(
            ADMIN_QUESTION_PHRASING_POOL,
            min(ADMIN_QUESTION_SAMPLE_SIZE, len(ADMIN_QUESTION_PHRASING_POOL)),
        )
        question_variety_hint = (
            ...
            "чтобы в чате было понятно, о чём речь; (2) вопрос к аудитории в одном из стилей ниже. "
            "Нельзя выводить в question только короткую фразу без контекста (например, только «Как вы это восприняли?» — недопустимо). "
            "Варианты окончания вопроса (выбери один по смыслу или свой в том же духе, после изложения новости): "
            + " | ".join(question_phrasing_sample)
            + ".\n\n"
        )
        prompt = (
