...
class OpenAIClient:
    ...
    def generate_discussion_messages(
        ...
        *,
        ...
        prompt = (
            "Сгенерируй обсуждение новости для чата.\n"
            "Верни JSON строго вида:\n"
            "{\"question\": \"...\", \"replies\": [\"...\", ...]}\n"
            f"Количество ответов: {replies_count}\n"
            "Ответы должны быть разными, естественными, без формальностей.\n"
            "Роли участников:\n"
            f"{roles_text}\n\n"
            "Новость:\n"
            f"{news_text}"
    ...
    def generate_user_reply(
        ...
        *,
        ...
        prompt = (
            "Ты участник живого чата. Ответь естественно и кратко (1-2 предложения). "
            "Без фраз 'я бот', без ссылок и без призывов подписаться.\n"
            f"Роль: {role_label}\n\n"
            "Последние сообщения чата:\n"
            f"{context_block}\n\n"
            "Сообщение пользователя, на которое отвечаем:\n"
            f"{source_text}"
        )
        text, in_tokens, out_tokens, total_tokens = self._with_retries(
            lambda: self._responses_text(self.system_prompt, prompt)
